...
masterObject.alarm = 1       // what alarm we are acting on by default
...
masterObject.Sunday.amFlag[2] = 1
masterObject.Sunday.amFlag[3] = 1
...
masterObject.Monday.amFlag[2] = 1
masterObject.Monday.amFlag[3] = 1
...
masterObject.Tuesday.amFlag[2] = 1
masterObject.Tuesday.amFlag[3] = 1
...
masterObject.Wednesday.amFlag[2] = 1
masterObject.Wednesday.amFlag[3] = 1
...
masterObject.Thursday.amFlag[2] = 1
masterObject.Thursday.amFlag[3] = 1
...
masterObject.Friday.amFlag[2] = 1
masterObject.Friday.amFlag[3] = 1
...
masterObject.Saturday.amFlag[2] = 1
masterObject.Saturday.amFlag[3] = 1
...
function getText( url )
{
  ...
  temp.type = 'GET'
  temp.dataType = 'html'
  temp.success = ajaxSuccessData
  temp.async = false
  temp.error  = ajaxError_1
  $.ajax(temp);

/*
  ie 7 does not like in line object construction -- though it is legal java script
  $.ajax(
  {
     url: url,
     type: 'GET',
...
function packConvert( object, index )
{
  ...
  // temp is time in the form of hr:min 
  // convert 12:xx to 00:xx
 
  
     if( ((index == 1 ) ||( index == 3 )) && ( (temp[0]== '1') &&
                            ( temp[1] == '2' ) &&
                            ( temp[3] == '0' ) &&
                            ( temp[4] == '0') ) &&
                            ( object.amFlag[index ] == 0 ) )
     {
        
        returnValue = "24:00"
     }
     else if( ( (index == 0)  ||(index == 2 )) && ( (temp[0]== '1') &&
                            ( temp[1] == '2' ) &&
                            ( temp[3] == '0' ) &&
                            ( temp[4] == '0') ) &&
                            ( object.amFlag[index ] == 0 ) )
...
function parseTimeField( inputData)
{
 ...
 var temp1
 var returnValue;
 var hrMap = [ "12","01","02","03","04","05","06","07","08","09","10","11","12" ]
 ...
 returnValue = hrMap[hrs] + ":"+ fields[1]
...
function validateTime( inputVal, dataObj )
{
  ...
  if( inputVal.val() > "12:59" )
  {
     alert(inputVal.val()+" is greater than 12:59 try again");
     inputVal.val("12:59");
...
function determineTimeSpan( dataObj, reference )
{
   ...
   if( reference == 0 )
   {
      ...
      returnValue.startTime = determineMinutesA( startObj, startSelect);
      returnValue.endTime = determineMinutesB( endObj, endSelect);
...
function determineMinutesA( startObj, selectObj)
{
   ...
   temp = $(startObj).attr("value")
   if( ( ( temp[0] == '1') && (temp[1] == '2') ))
   {
      
      temp = "00:"+temp[3]+temp[4];
...
function determineMinutesB( startObj, selectObj)
{
   var hrs
   var minutes
   var fields
  ...
  if( (selectValue == 1 ) && ( ( temp[0] == '1') && (temp[1] == '2') ))
   {
     temp = "00:"+ temp[3]+temp[4]
   }
    if( (selectValue == 0) && ( ( temp[0] == '1') && (temp[1] == '2') ))
   {
...
function convert12( input )
{
  ...
  {
    returnValue = "00:"+input[3]+input[4];
  }
  else
  {
     returnValue = input;
...
function convertTo12( input )
{
  var returnValue

  if( (input[0] == '0') && ( input[1] == '0') )
  {
    returnValue = "12:"+input[3]+input[4];
  }
  else
  {
     returnValue = input;
